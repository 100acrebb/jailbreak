...
local CTRL_NEXT2 = IN_MOVERIGHT;
local CTRL_PREV2 = IN_MOVELEFT;

hook.Add( "KeyPress", "JB.KeyPress.HandleSpectateControls", function(p,key)
	
	
	
	-- don't override if we're in pure spec mode.
	--if (p:Team() == TEAM_SPECTATOR) then
	--	return
	--end

	if ( key == CTRL_NEXT or key == CTRL_PREV or key == CTRL_CHANGE or key == CTRL_NEXT2 or key == CTRL_PREV2 ) and p:GetObserverMode() != OBS_MODE_NONE then
		...
		local old=p.spec;
		if (key == CTRL_NEXT or (key == CTRL_NEXT2 and p:GetObserverMode() == OBS_MODE_CHASE)) then
			p.spec = p.spec+1;
			if p.spec > #canspec then
				p.spec = 0;
			end
		elseif (key == CTRL_PREV or (key == CTRL_PREV2 and p:GetObserverMode() == OBS_MODE_CHASE)) then
			p.spec = p.spec-1;
			if p.spec < 1 then
				p.spec = #canspec;
			end
